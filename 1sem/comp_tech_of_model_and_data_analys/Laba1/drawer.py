...
class Line:
    def __init__(self, x0 : float, y0 : float, x1 : float, y1 : float, line_width : float):
        self.p1 = Point(x0, y0)
        self.p2 = Point(x1, y1)
        self.line_width = line_width

def draw(big_big_list : list):
    if len(big_big_list) == 0:
        raise Exception("Error: empty lines array.")
    for line in big_big_list:
        plt.plot([line.p1.x, line.p2.x],
                 [line.p1.y, line.p2.y], color="black", linewidth=line.line_width)
    plt.savefig("Data\\grid.png")

def read_file(relative_path_input : str) -> list:
